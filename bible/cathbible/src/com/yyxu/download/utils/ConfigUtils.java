...
public class ConfigUtils {
	...
	public static void storeURL(Context context, int index, String url, String path) {
		setString(context, KEY_URL + index, url + "###" + path);
	}
	...
	public static List<Map<String,String>> getURLArray(Context context) {
		...
		for (int i = 0; i < URL_COUNT; i++) {
			if (!TextUtils.isEmpty(getURL(context, i))) {
                String str = getURL(context, i);
                if(str.contains("###")) {
                    Map<String,String> map = new HashMap<String, String>();
                    map.put("url",str.split("###")[0]);
                    map.put("path",str.split("###")[1]);
                    urlList.add(map);
                }
			}
		}
