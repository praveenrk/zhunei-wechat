var todoCtrl = $.mvc.controller.create('todo', {
    ...
    default:function() {
        todo.fetchAll(function(all) {
            var active = all.filter(function(obj) {
                return obj.isComplete == false && obj.isArchived == false
            });
            var completed = all.filter(function(obj) {
                return obj.isComplete == true
            });
            var archived = all.filter(function(obj) {
                return obj.isArchived == true
            });

            $("#leftTodo").html($.template('list_tpl', {
                title: 'Active',
            ...
            $("#completedTodo").html($.template('list_tpl', {
                title: 'Completed',
                listCSS: 'completed  mainScreen',
                items: completed,
                state: 'complete',
                checked: 'checked',
                archiveText: 'Archive'
            }));
            $("#archivedTodo").html($.template('list_tpl', {
                title: 'Archived',
                listCSS: 'archived archiveScreen',
                items: archived,
                state: 'archived',
                checked: '',
                archiveText: 'Delete'
